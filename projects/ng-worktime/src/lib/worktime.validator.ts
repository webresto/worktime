...
export class WorkTimeValidator {
  ...
  static getMaxOrderDate(restictions:RestrictionsOrder) {
    ...
    /*this.restriction = restictions; // сохраняем объект с ограничениями
        this.dateMax = this.ui.formatDate(Date.now() + restictions.periodPossibleForOrder * 60000, 'yyyy-MM-dd', 'en'); // максимальная дата заказа
        if (!this.restriction.workTime) {
          (<FormGroup>this.orderForm.controls.deliveryTimeInfo).controls.deliveryType.setValue('fast');
          this.disableButtonVariableDelivery$.next({ fast: false, toTime: true });
        } else {
